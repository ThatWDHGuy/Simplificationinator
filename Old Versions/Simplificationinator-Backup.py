...
#####################################################################################################################
	...
	for oltName in oltAddNames:
		...
		for priority, logList in pdict.iteritems():
			for tup in logList:
				if tup[0] == codeMeshSdpBindingCreate:
		...
		for name in sdpSwitches:
			if sdpSwitches.count(name) > highestCount:
				...
				oltAddedTo = name
		logsOut.append(["addOLT", str(oltName + " on " + oltAddedTo)])
		...
		for priority, logList in pdict.iteritems():
			...
			while i < len(logList):
				...
				elif tup[0] == codeMeshSdpBindingCreate:
				...
				elif tup[0] == codeOltMacNameCreate:
...
#####################################################################################################################
	...
	for priority, logList in pdict.iteritems():
		for tup in logList:
			if tup[0] == codeHandoverMacNameCreate:
				groups = r.match(tup[1]).groups()
					...
					while i < len(logList):
						...
						if tup[0] == codeBSiteCreate:
							...
							if groups[9] not in switchesMacMadeOn and groups[10] in switchesMacMadeOn:
								...
								#print groups[0], groups[1], switchesMacMadeOn, tup[1]
						elif tup[0] == codeStaticMacCreate:
	...
	for oltName in oltsRemoved:
		...
		for priority, logList in pdict.iteritems():
			...
			while i < len(logList):
				tup = logList[i]
				groups = r.match(tup[1]).groups()
				if tup[0] == codeOltMacNameDelete:
					switchesMacRemovedFrom.append(groups[6])
					logList.pop(i)
					i -= 1
				elif tup[0] == codeVplsDelete:
					logList.pop(i)
					i -= 1
				i += 1
		...
		for priority, logList in pdict.iteritems():
			...
			while i < len(logList):
				tup = logList[i]
				groups = r.match(tup[1]).groups()
				if tup[0] == codeHandoverMacNameDelete:
					...
					while i < len(logList):
						...
						if tup[0] == codeBSiteDelete:
							...
							if groups[9] not in switchesMacMadeOn and groups[10] in switchesMacMadeOn:
								...
								#print groups[0], groups[1], switchesMacMadeOn, tup[1]
						elif tup[0] == codeStaticMacDelete:
							#print groups[12], groups[14], switchesMacMadeOn
							if groups[12] not in switchesMacMadeOn and groups[14] in switchesMacMadeOn:
								# valid
								logList.pop(i)
						...
						elif tup[0] == codeOltMacNameDelete:
